...
uint64_t shift_bit(uint64_t bitboard, int vy, int vx, int zy, int zx)
{ // wenn (vy, vx)==1
    // clear bit at (vy, vx)
...
std::vector<uint64_t> gcTtXxQq(const uint64_t bitboard, const uint64_t this_players_pieces, const uint64_t other_players_pieces, std::vector<std::pair<int, int>> directions)
{
    ...
    for (int y = 0; y < 8; ++y)
    {
        for (int x = 0; x < 8; ++x)
        {
            if (is_one_at_this_index(bitboard, yx_zu_index(y, x)))
            {
        ...
        for (const auto &direction : directions)
        {
            ...
            for (int i = 1; i < 8; ++i)
            {
                ...
                if (is_in_board(y_new, x_new))
                {
                    ...
                    else if (is_one_at_this_index(this_players_pieces, yx_zu_index(y_new, x_new)))
                    {
                        break;
                    }
                    else
...
std::vector<std::pair<uint64_t, uint64_t>> gcZz(const uint64_t Zz_bitboard, const uint64_t Tt_bitboard, const uint64_t this_players_pieces, const uint64_t other_players_pieces)
{
    ...
    for (int y = 0; y < 8; ++y)
    {
        for (int x = 0; x < 8; ++x)
        {
            if (is_one_at_this_index(Zz_bitboard, yx_zu_index(y, x)))
            {
                positions.push_back({y, x});
            }
        }
...
std::vector<uint64_t> gcKk(const uint64_t king_bitboard, const uint64_t this_players_pieces)
{
    ...
    for (int y = 0; y < 8; ++y)
    {
        for (int x = 0; x < 8; ++x)
        {
            if (is_one_at_this_index(king_bitboard, yx_zu_index(y, x)))
            {
                king_positions.push_back({y, x});
            }
        }
    }
    // modify boards
...
std::vector<uint64_t> gcLl(const uint64_t knight_bitboard, const uint64_t this_players_pieces)
{
    ...
    for (int y = 0; y < 8; ++y)
    {
        for (int x = 0; x < 8; ++x)
        {
            if (is_one_at_this_index(knight_bitboard, yx_zu_index(y, x)))
            {
                knight_positions.push_back({y, x});
            }
        }
    }
    // modify boards
...
std::vector<std::vector<uint64_t>> gcBb(int player, const uint64_t pawn_bitboard, const uint64_t this_players_pieces, const uint64_t this_players_Ff, const uint64_t other_players_pieces, const uint64_t other_players_Ff)
{
    ...
    for (int y = 0; y < 8; ++y)
    {
        for (int x = 0; x < 8; ++x)
        {
            if (is_one_at_this_index(pawn_bitboard, yx_zu_index(y, x)))
            {
                pawn_positions.push_back({y, x});
            }
        }
    }
    //
...
std::vector<std::pair<uint64_t, uint64_t>> gorcZz(const uint64_t Zz_bitboard, const uint64_t Tt_bitboard, const uint64_t this_players_pieces, const uint64_t other_players_pieces)
{
    ...
    if (positions.empty())
    {
        std::vector<std::pair<uint64_t, uint64_t>> empty_vector;
        return empty_vector;
    }
    // collect coordinates of moves
    for (const auto &position : positions)
...
std::vector<std::vector<uint64_t>> gorcBb(int player, const uint64_t pawn_bitboard, const uint64_t this_players_pieces, const uint64_t this_players_Ff, const uint64_t other_players_pieces, const uint64_t other_players_Ff)
{
    ...
    {
        std::vector<std::vector<uint64_t>> empty_vector;
        return empty_vector;
    }
    //
    uint64_t all_pieces = this_players_pieces | other_players_pieces;
...
public:
    ...
    void print_board() const
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                ...
                {
                    std::cout << BEIGE_TEXT << "b" << RESET_TEXT;
                }
                else if (board[i][j] == 2)
                {
                    std::cout << BEIGE_TEXT << "L" << RESET_TEXT;
                }
                else if (board[i][j] == 3)
                {
                    std::cout << BEIGE_TEXT << "X" << RESET_TEXT;
                }
                else if (board[i][j] == 4 || board[i][j] == 7)
                {
                    std::cout << BEIGE_TEXT << "T" << RESET_TEXT;
                }
                else if (board[i][j] == 5)
                {
                    std::cout << BEIGE_TEXT << "Q" << RESET_TEXT;
                }
                else if (board[i][j] == 6 || board[i][j] == 8)
                {
                    std::cout << BEIGE_TEXT << "K" << RESET_TEXT;
                }
                else if (board[i][j] == -1 || board[i][j] == -9)
                {
                    std::cout << BROWN_TEXT << "b" << RESET_TEXT;
                }
                else if (board[i][j] == -2)
                {
                    std::cout << BROWN_TEXT << "L" << RESET_TEXT;
                }
                else if (board[i][j] == -3)
                {
                    std::cout << BROWN_TEXT << "X" << RESET_TEXT;
                }
                else if (board[i][j] == -4 || board[i][j] == -7)
                {
                    std::cout << BROWN_TEXT << "T" << RESET_TEXT;
                }
                else if (board[i][j] == -5)
                {
                    std::cout << BROWN_TEXT << "Q" << RESET_TEXT;
                }
                else if (board[i][j] == -6 || board[i][j] == -8)
                {
                    std::cout << BROWN_TEXT << "K" << RESET_TEXT;
                }
                else
                {
                    std::cout << " ";
            ...
            for (int p = 0; p < 8; ++p)
            {
                for (int o = 0; o < 8; ++o)
                {
                    if (is_one_at_this_index(this->B, yx_zu_index(p, o)) || is_one_at_this_index(this->F, yx_zu_index(p, o)))
                    {
                        ...
                        if (p + 1 <= 7 && o + 1 <= 7)
                        {
                            if (is_one_at_this_index(this->B, yx_zu_index(p + 1, o + 1)) || is_one_at_this_index(this->F, yx_zu_index(p + 1, o + 1)))
                            {
                                val += -10;
                            }
                        }
    ...
    std::vector<Board> gcYy(int player, const uint64_t this_players_pieces, const uint64_t all_pieces)
    {
        ...
        for (int y = 0; y < 8; ++y)
        {
            for (int x = 0; x < 8; ++x)
            {
                if (is_one_at_this_index(Yy_bitboard, yx_zu_index(y, x)))
                {
                    Yy_positions.push_back({y, x});
                }
            }
        }
        //
    ...
    std::vector<Board> generate_children(int playerk)
    {
        ...
        if (playerk == 6)
        {
            for (int y = 0; y < 8; ++y)
            {
                for (int x = 0; x < 8; ++x)
                {
                    if (is_one_at_this_index(this->f, yx_zu_index(y, x)))
                    {
                        positions_Ff.push_back({y, x});
                    }
                }
            }
        ...
        if (playerk == 6)
        {
            if (this->b != 0)
            {
                for (const auto &board : gcBb(1, this->b, white_pieces, this->f, black_pieces, this->F))
                {
                    ...
                    if (child_black_pieces != black_pieces)
                    {
                        child.schlagen = true;
                    }
                    else
            ...
            if (this->y != 0)
            {
                for (Board child_Yy : gcYy(6, white_pieces, all_pieces))
                {
        ...
        else
        {
            if (this->B != 0)
            {
                for (const auto &board : gcBb(-1, this->B, black_pieces, this->F, white_pieces, this->f))
                {
                    ...
                    if (child_white_pieces != white_pieces)
                    {
                        child.schlagen = true;
                    }
                    else
            ...
            if (this->Y != 0)
            {
                for (Board child_Yy : gcYy(-6, black_pieces, all_pieces))
                {
                    children.push_back(child_Yy);
                }
            }
    ...
    std::vector<Board> generate_one_random_child(int playerk)
    {
        ...
        for (int i = 0; i < anz_Bb; ++i)
        {
            for (int j = 0; j < Bb_c; ++j)
            {
                pieces.push_back(1);
            }
        }
        for (int i = 0; i < anz_Ll; ++i)
        {
            for (int j = 0; j < Ll_c; ++j)
            {
                pieces.push_back(2);
            }
        }
        for (int i = 0; i < anz_Xx; ++i)
        {
            for (int j = 0; j < Xx_c; ++j)
            {
                pieces.push_back(3);
            }
        }
        for (int i = 0; i < anz_Tt; ++i)
        {
            for (int j = 0; j < Tt_c; ++j)
            {
                pieces.push_back(4);
            }
        }
        for (int i = 0; i < anz_Qq; ++i)
        {
            for (int j = 0; j < Qq_c; ++j)
            {
                pieces.push_back(5);
            }
        }
        for (int i = 0; i < anz_Kk; ++i)
        {
            for (int j = 0; j < Kk_c; ++j)
            {
                pieces.push_back(6);
            }
        }
        for (int i = 0; i < anz_Zz; ++i)
        {
            for (int j = 0; j < Zz_c; ++j)
            {
                pieces.push_back(7);
            }
        }
        for (int i = 0; i < anz_Yy; ++i)
        {
            for (int j = 0; j < Yy_c; ++j)
            {
                pieces.push_back(8);
            }
        }
            ...
            if (playerk == 6)
            {
                if (chosen_piece == 1)
                {
                    for (const auto &board : gorcBb(1, this->b, white_pieces, this->f, black_pieces, this->F))
                    {
                        ...
                        { // Zug war 2 nach vorne, kein Schlagen
                            child.B = this->B;
                            child.L = this->L;
                            child.X = this->X;
                            child.T = this->T;
...
std::vector<std::vector<int>> convert_board_to_vector(const Board &board)
{
    ...
    for (int i = 0; i < 64; i++)
    {
        ...
        if (board.B & (1ULL << i))
            result[row][col] = -1;
        else if (board.L & (1ULL << i))
            result[row][col] = -2;
        else if (board.X & (1ULL << i))
            result[row][col] = -3;
        else if (board.T & (1ULL << i))
            result[row][col] = -4;
        else if (board.Q & (1ULL << i))
            result[row][col] = -5;
        else if (board.K & (1ULL << i))
            result[row][col] = -6;
        else if (board.Z & (1ULL << i))
            result[row][col] = -7;
        else if (board.Y & (1ULL << i))
            result[row][col] = -8;
        else if (board.F & (1ULL << i))
            result[row][col] = -9;

        else if (board.b & (1ULL << i))
            result[row][col] = 1;
        else if (board.l & (1ULL << i))
            result[row][col] = 2;
        else if (board.x & (1ULL << i))
...
Board *convert_vector_to_board(std::vector<std::vector<int>> vector)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        std::reverse(vector[i].begin(), vector[i].end());
    }
    //
    board->b = 0ULL;
    board->l = 0ULL;
    ...
    for (int row = 0; row < 8; row++)
    {
        for (int col = 0; col < 8; col++)
        {
            ...
            if (piece == -1)
                board->B |= (1ULL << (row * 8 + col));
            else if (piece == -2)
                board->L |= (1ULL << (row * 8 + col));
            else if (piece == -3)
                board->X |= (1ULL << (row * 8 + col));
            else if (piece == -4)
                board->T |= (1ULL << (row * 8 + col));
            else if (piece == -5)
                board->Q |= (1ULL << (row * 8 + col));
            else if (piece == -6)
                board->K |= (1ULL << (row * 8 + col));
            else if (piece == -7)
                board->Z |= (1ULL << (row * 8 + col));
            else if (piece == -8)
                board->Y |= (1ULL << (row * 8 + col));
            else if (piece == -9)
                board->F |= (1ULL << (row * 8 + col));
    ...
    std::vector<std::vector<int>> player(std::vector<std::vector<int>> &pos)
    {
        ...
        while (true)
        {
            ...
            if (!special)
            {
                ...
                else if (boardcopy[vy][vx] == -7)
                {
                    boardcopy[zy][zx] = -4;
                }
                else if (boardcopy[vy][vx] == 8)
                {
                    boardcopy[zy][zx] = 6;
                }
                else if (boardcopy[vy][vx] == -8)
            ...
            else
            {
                ...
                for (const auto &s : legal_moves)
                {
                    print_vector_board(s);
                }
                std::cout << "---------------------------------" << std::endl;
...
public:
    ...
    int minimax(int alpha, int beta, bool max_player, const int max_depth)
    {
        ...
        if (!this->expanded)
        {
            this->children = this->expand_node();
            this->expanded = true;
        }
    ...
    void sort(bool max_player)
    {
        if (this->expanded)
        {
            ...
            {
                if (!child.value_not_none)
                {
                    none_children.push_back(child);
                }
                else
                {
                    not_none_children.push_back(child);
                }
            }
            //
            if (max_player)
            {
                std::sort(not_none_children.begin(), not_none_children.end(), [](const MinimaxNode &a, const MinimaxNode &b)
                ...
                //
                for (MinimaxNode &child : not_none_children)
                {
                    child.sort(false);
                }
            }
...
public:
    ...
    Board *get_move()
    {
        ...
        while (true)
        {
            ...
            if (vergangene_zeit.count() >= max_time)
            {
                break;
            }
            ...
            *move = *new_move;
            ...
            //
            for (const MinimaxNode &child : root_node.children)
            {
...
public:
    ...
    void sort(bool max_player)
    {
        if (this->expanded)
        {
            ...
            if (max_player)
            {
                std::sort(not_none_children.begin(), not_none_children.end(), [](const MinimaxNode2 &a, const MinimaxNode2 &b)
                          { return a.value > b.value; });
                //
                sorted_children.insert(sorted_children.end(), not_none_children.begin(), not_none_children.end());
                sorted_children.insert(sorted_children.end(), none_children.begin(), none_children.end());
                this->children = sorted_children;
                //
                for (MinimaxNode2 &child : not_none_children)
                {
                    child.sort(false);
                }
            }
    ...
    Board *get_move()
    {
        ...
        while (true)
        {
            ...
            *move = *new_move;
            ...
            //
            for (const MinimaxNode2 &child : root_node.children)
            {
                std::cout << child.value;
                std::cout << ", ";
            }
            ...
            this->root_node.sort(true);
            //
            for (MinimaxNode2 &child : root_node.children)
            {
                std::cout << child.value;
                std::cout << ", ";
            }
...
public:
    ...
    void sort(bool max_player)
    {
        if (this->expanded)
        {
            ...
            if (max_player)
            {
                std::sort(not_none_children.begin(), not_none_children.end(), [](const MinimaxNode3 &a, const MinimaxNode3 &b)
                          { return a.value > b.value; });
                //
                sorted_children.insert(sorted_children.end(), not_none_children.begin(), not_none_children.end());
                sorted_children.insert(sorted_children.end(), none_children.begin(), none_children.end());
                this->children = sorted_children;
                //
                for (MinimaxNode3 &child : not_none_children)
                {
                    child.sort(false);
                }
            }
...
public:
    ...
    Board *get_move()
    {
        ...
        while (true)
        {
            ...
            //
            for (const MinimaxNode3 &child : root_node.children)
            {
                std::cout << child.value;
                std::cout << ", ";
            }
            ...
            this->root_node.sort(true);
            //
            for (MinimaxNode3 &child : root_node.children)
            {
                std::cout << child.value;
                std::cout << ", ";
            }
...
public:
    ...
    int play()
    {
        ...
        while (true)
        {
            ...
            {
                std::cout << "NULLPOINTER2" << std::endl;
                delete new_board;
            }
            //
            if (!(new_board == nullptr))
